...
# Runtime Manager
# -----------------------------------------------------------------------------
# mise manages node, ruby, python and reads .nvmrc / .ruby-version / .python-version
# / .tool-versions for per-directory switching.
...
# Find files
alias ff='find . -type f -name'  # Usage: ff "*.js"  (fd binary handles directory search)
...
_postgres_data_dir() {
  ls -d "$HOME/Library/Application Support/Postgres/var-"* 2>/dev/null | sort -V | tail -1
}
psql-start() {
  local dir; dir="$(_postgres_data_dir)"
  [[ -z "$dir" ]] && { echo "No Postgres.app data dir found" >&2; return 1; }
  pg_ctl -D "$dir" start
}
psql-stop() {
  local dir; dir="$(_postgres_data_dir)"
...
# -----------------------------------------------------------------------------
# Shell Plugins (loaded last — syntax-highlighting must wrap everything else)
# -----------------------------------------------------------------------------
